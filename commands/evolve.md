...
用途：将相关的本能（instincts）聚类成技能、命令或代理。

用法：
  /evolve                    # 分析所有本能并建议进化
  /evolve --domain testing   # 只进化测试领域的本能
  /evolve --dry-run          # 预览将要创建的内容而不实际创建
  /evolve --threshold 5      # 需要5+个相关本能才能聚类

进化规则：
1. → 命令（Command）：当本能描述用户显式请求的操作时
   - 多个关于"当用户要求..."的本能
   - 触发器如"当创建新X时"
   - 遵循可重复序列的本能

2. → 技能（Skill）：当本能描述应自动触发的行为时
   - 模式匹配触发器
   - 错误处理响应
   - 代码风格强制

3. → 代理（Agent）：当本能描述受益于隔离的复杂多步骤过程时
   - 调试工作流
   - 重构序列
   - 研究任务

执行方法：
运行 instinct CLI 工具分析本能并生成高级结构
